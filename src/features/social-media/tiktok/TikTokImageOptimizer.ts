...
import { TIKTOK_IMAGE_PROFILE } from './TikTokImageProfile'

export const imageProfile = TIKTOK_IMAGE_PROFILE

export async function prepareTikTokImage(
  file: File,
  destinationKey: string,
): Promise<SocialImagePreparationResult | null> {
  const destination = TIKTOK_IMAGE_PROFILE[destinationKey]
  ...
  return {
    ...
    outputFileName: buildSocialImageFileName(file.name, 'tiktok'),
  }
}
