...
import { WHATSAPP_IMAGE_PROFILE } from './WhatsAppImageProfile'

export const imageProfile = WHATSAPP_IMAGE_PROFILE

export async function prepareWhatsAppImage(
  file: File,
  destinationKey: string,
): Promise<SocialImagePreparationResult | null> {
  const destination = WHATSAPP_IMAGE_PROFILE[destinationKey]
  ...
  return {
    ...
    outputFileName: buildSocialImageFileName(file.name, 'whatsapp'),
  }
}
